...
@mdempski propos 2014 add unsafe.add, analog exist (unexported) runtim function
```
func add(p unsafe.Pointer, x uintptr) unsafe.Pointer {
       return unsafe.Pointer(uintptr(p) + x)
}
```
#19367 ad unsafe.slice, #395 slice array conversion. unsafe.add time would good bundl peopl want clean use unsaf one time.
issu propos unsafe.add, probabl function, mayb method pointer? matthew' doc discuss tradeoff there.
