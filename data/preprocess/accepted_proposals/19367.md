...
In contrast, punning an `unsafe.Pointer` to a slice does not allocate any kind of backing store — it *only* returns a header that refers to the existing data. That feels like a different operation to me — more like a conversion than a `make`.
