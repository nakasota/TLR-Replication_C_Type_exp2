=== Fetching Proposal: MDU6SXNzdWU4NTM5OTg2MjA= ===
Issue URL: https://github.com/golang/go/issues/45460
...
[RFC 9110 9.3.6](https://www.rfc-editor.org/rfc/rfc9110#name-connect) states that a proxy should connect to the "server identified by the request target". It doesn't say anything about the `Host` header that I can see, and I can't find any examples of using different values in the target and the `Host` header.

I did find a [2009 ietf-http-wg thread](https://lists.w3.org/Archives/Public/ietf-http-wg/2009AprJun/0696.html) discussing the role of the `Host` header in `CONNECT` requests which seems to have concluded that the header is required, but that proxies should either ignore it or validate that it exactly matches the request target.
