...
--- Comment #12 by erikdubbelboer ---
That is great to hear. I already suggested an implementation here: https://go-review.googlesource.com/c/tools/+/354010 (and here: https://github.com/golang/tools/pull/342)

--- Comment #13 by ernado ---
There is a little chance of false positive in places that had to provide compatibility with such erroneous format.

For example, we can find such in [LLVM code](https://github.com/llvm/llvm-project/blob/main/openmp/runtime/tools/lib/Build.pm#L34) (which is still a bug), in some [weird Apache tests](https://sourcegraph.com/search?q=context:global+repo:apache/.*+-file:.*test.*+yyyy-dd-mm&patternType=literal) and other places (?).
...
--- Comment #15 by ernado ---
Agree, thank you. 

--- Comment #16 by erikdubbelboer ---
...
--- Comment #21 by erikdubbelboer ---
> If "2006-01-02" is worthy of a vet check, is it also worthy of a constant? Maybe `time.RFC3339Date`, `time.ISO8601Date` or just `time.ISODate`?

Seems like this is going to happen: https://github.com/golang/go/issues/52746

--- Comment #22 by AlexanderYastrebov ---
Should this be closed given https://go.dev/cl/354010 is merged?

--- Comment #23 by seankhliao ---
[CL 354010](https://go.dev/cl/354010) adds the `timeformat` analysis pass, but it's not part of `cmd/vet` yet.
