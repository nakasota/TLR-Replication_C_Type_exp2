...
--- Comment #17 by aofei ---
Since we can't use `GOCACHE`, then I think `GOMODCACHE` is our best option. Because we already have some things that are using `MODCACHE`, such as `go clean -modcache`. I think `DIR` is weird (at least we have never used) and one `GOPATH` is enough, please don't bring more `PATH`s. 😂
