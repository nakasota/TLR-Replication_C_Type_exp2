...
--- Comment #24 by dmitshur ---
Re-opening as [CL 224588](https://golang.org/cl/224588) was rolled back in [CL 228200](https://golang.org/cl/228200).

--- Comment #25 by gopherbot ---
Change https://golang.org/cl/228101 mentions this issue: `time/tzdata: new package`

--- Comment #26 by xtonyjiang ---
...
1. I wrote [an ugly script](https://github.com/NuxiNL/cloudlibc/blob/master/src/libc/locale/timezone/gen_tzdata.py) that takes all of the IANA tzdata text files and parses them. In addition to that, it computes some implicit properties that are needed for our algorithm to run quickly (e.g., the amount of DST that was being used right before a location moved from one time zone to another).
2. This script stomps out [this C header file](https://github.com/NuxiNL/cloudlibc/blob/master/src/libc/locale/timezone/tzdata.h) containing all of the unflattened definitions in a compilable format. The definitions for each of the struct types used can be found [here](https://github.com/NuxiNL/cloudlibc/blob/master/src/common/locale.h#L246-L274).
3. Then I wrote implementations of [mktime](https://github.com/NuxiNL/cloudlibc/blob/master/src/libc/time/mktime_l.c) and [localtime](https://github.com/NuxiNL/cloudlibc/blob/master/src/libc/time/localtime_l.c) that work on top of these data structures ([common subroutines](https://github.com/NuxiNL/cloudlibc/blob/master/src/libc/time/time_impl.h)).
4. Last but not least, I wrote some scripts around `zdump -v` to generate [long lists of unit tests](https://github.com/NuxiNL/cloudlibc/blob/master/src/libc/time/localtime_l_test.cc). The time zones used in these tests are the ones that I discovered were problematic in some sense (e.g., countries that moved across the international date line, [British Double Summer Time](https://en.wikipedia.org/wiki/British_Summer_Time)).
