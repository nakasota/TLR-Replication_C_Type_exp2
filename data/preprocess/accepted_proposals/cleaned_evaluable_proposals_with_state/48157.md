...
@josharian, no `Context` method is needed. Tests and benchmarks can already do cooperative graceful shutdown via the [`Deadline`](https://pkg.go.dev/testing#T.Deadline) method — we just need to allow more overhead today because all of the tests end up shutting down at once when the timeout is about to expire.
