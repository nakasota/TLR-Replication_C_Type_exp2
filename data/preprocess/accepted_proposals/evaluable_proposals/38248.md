...
--- Comment #16 by cherrymui ---
Sorry about being late.

Does `go:wasmimport` work for arbitrary modules and functions, or only for a list of special functions?

A bit bikeshedding: is "go" the best name for wasm_exec.js? From the Go code side, everything is go _except_ the imported functions. It is true that the module is currently named "go", but we can change that. Maybe "jsgo" or  "gojs"?


--- Comment #17 by neelance ---
> Does go:wasmimport work for arbitrary modules and functions, or only for a list of special functions?
...
--- Comment #30 by vedantroy ---
@rsc @cherrymui How do the [new Go ABI changes](https://go.googlesource.com/go/+/84825599dc2592e9e9f5489323fa7b47534fc391) (not sure if that's the right term) affect this proposal? I had a working version of this proposal [here](https://go-review.googlesource.com/c/go/+/290112) before the ABI changes, but it looks there's been significant changes in the past few months.
