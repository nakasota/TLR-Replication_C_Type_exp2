...
For transparency: we have a pull request pending in moby, in which we're able to work around this issue; there was some interaction whether or not a change in Go's package would still be needed, but from that conversation, it looks like having this change in Go would still be beneficial (and allow us to have a cleaner way to work around the issue (as well as more easily allow other consumers of the package to implement); see https://github.com/moby/moby/pull/43185#issuecomment-1032867691

--- Comment #27 by gopherbot ---
Change https://go.dev/cl/514235 mentions this issue: `archive/tar: add FileInfoNames interface`

--- Comment #28 by gopherbot ---
Change https://go.dev/cl/518056 mentions this issue: `archive/tar: correct value passed to Uname method`

--- Comment #29 by rsc ---
Reopening because the uid and gid arguments to Uname and Gname are Unix-specific and also don't make that much sense: if the FileInfo represents info about a single file, then why would you ask _the file_ how to convert an arbitrary Uid or Gid value to a string? Instead, the method that would make sense on a FileInfo is:

Uname() (string, error)
Gname() (string, error)

and for that matter maybe we should also add
