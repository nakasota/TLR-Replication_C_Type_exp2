=== Fetching Proposal: I_kwDOAWBuf85BsCpv ===
Issue URL: https://github.com/golang/go/issues/50599
...
On POSIX platforms, the [`PWD` environment variable](https://pubs.opengroup.org/onlinepubs/9699919799/basedefs/V1_chap08.html#tag_08_03) represents (emphasis mine) “<strong>an</strong> absolute pathname of the current working directory.”
...
* #33598 (“unexpected stale targets” on macOS)
...
--- Comment #9 by philpennock ---
Should linters warn if an variable `c` of type `os/exec.Cmd` has `c.Environ()` called on it _before_ `c.Dir` is mutated?
