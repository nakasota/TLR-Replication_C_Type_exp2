=== Fetching Proposal: MDU6SXNzdWU3OTcwMDg0MDk= ===
Issue URL: https://github.com/golang/go/issues/43993
...
IMO `reflect.Value` should be treated as analogous to a pointer type. We treat `&x` as semantically different from `x`,[¹](https://play.golang.org/p/cPgs5BJdmb0) so we should also treat `reflect.ValueOf(x)` as semantically different from `x` itself.
