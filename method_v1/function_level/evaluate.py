...
def select_function_level_output_dir():
    ...
    if not output_dirs:
        raise RuntimeError('No output directories found in method_v1/function_level/output/. Please run function-level main.py first to generate outputs.')
...
def main():
    ...
    # Load both directory and file-level results from the file-level output directory
    dir_eval = {}
    file_eval = {}
    if selected_file_level:
        file_level_output_dir = Path(__file__).parent.parent / 'file_level' / 'output' / selected_file_level
