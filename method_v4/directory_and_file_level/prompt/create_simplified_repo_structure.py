...
def create_multiple_versions():
    ...
    # First create the full version (no file limiting)
    full_output = "simplified_repo_structure_full.txt"
    print(f"\n{'='*50}")
    print(f"Creating version: {full_output}")
    print(f"Max depth: unlimited, max_files_per_directory: unlimited (full repository)")
    try:
        structure = create_simplified_structure(input_file, full_output, None, max_files_per_directory=None)
        ...
        print(f"Error creating {full_output}: {e}")
    
    # Create the limited version (with file sampling)
    ...
    # Create depth-limited versions (all with file sampling)
    depth_versions = [
    ...
    for max_depth, output_file in depth_versions:
        print(f"\n{'='*50}")
        print(f"Creating version: {output_file}")
        print(f"Max depth: {max_depth}, max_files_per_directory: 5")
        try:
