...
# Select file-level output directory (coordinated or manual)
file_level_output_root = Path(__file__).parent.parent / 'directory_and_file_level_localization' / 'output'
...
if granularity_results:
    function_proposals = [p for p in proposal_files if granularity_results.get(p.name) == "function"]
    if len(function_proposals) < len(proposal_files):
        skipped_count = len(proposal_files) - len(function_proposals)
    ...
    proposal_files = function_proposals
