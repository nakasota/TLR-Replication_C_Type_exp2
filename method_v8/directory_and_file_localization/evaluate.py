#!/usr/bin/env python3
"""
Method v8: Directory and File Localization Evaluation

This script evaluates the directory and file localization results.
It compares the predicted relevant directories and files against ground truth data.
"""
...
def compute_file_metrics_within_correct_directories(pred_files, gt_files, pred_dirs, gt_dirs):
    """Compute file metrics only for predicted files within correctly predicted directories.
    
    This eliminates the penalty of predicting files in wrong directories while keeping
    the same recall denominator (all ground truth files).
    """
    # Find correctly predicted directories
    correct_dirs = pred_dirs & gt_dirs
    
    if not correct_dirs:
        # No correct directories, so no true positives possible
        # All ground truth files become false negatives
        return 0.0, 0.0, 0.0, 0, 0, len(gt_files)
    
    # Filter predicted files to only those in correct directories
    pred_files_in_correct_dirs = set()
    for file_path in pred_files:
        file_dir = str(Path(file_path).parent)
        if file_dir in correct_dirs:
            pred_files_in_correct_dirs.add(file_path)
    
    # Keep ALL ground truth files (don't filter by directory)
    # This ensures recall denominator remains the same
    
    # Compute metrics: predicted files in correct dirs vs all ground truth files
    return compute_metrics(pred_files_in_correct_dirs, gt_files)
...
def main():
    ...
    # File-level evaluation within correct directories
    all_file_prec_correct, all_file_rec_correct, all_file_f1_correct = [], [], []
    file_results_within_correct_dirs = {}
    ...
    for proposal_id, pred_files in llm_outputs.items():
        ...
        # File-level evaluation within correct directories
        file_precision_correct, file_recall_correct, file_f1_correct, file_tp_correct, file_fp_correct, file_fn_correct = compute_file_metrics_within_correct_directories(
            pred_files, gt_files, pred_dirs, gt_dirs
        )
        file_results_within_correct_dirs[proposal_id] = {
            'precision': file_precision_correct,
            'recall': file_recall_correct,
            'f1': file_f1_correct,
            'tp': file_tp_correct,
            'fp': file_fp_correct,
            'fn': file_fn_correct,
            'predicted': list(pred_files),
            'ground_truth': list(gt_files),
            'correct_directories': list(pred_dirs & gt_dirs)
        }
        all_file_prec_correct.append(file_precision_correct)
        all_file_rec_correct.append(file_recall_correct)
        all_file_f1_correct.append(file_f1_correct)
    ...
    # Calculate macro metrics for files within correct directories
    macro_file_precision_correct = sum(all_file_prec_correct) / len(all_file_prec_correct) if all_file_prec_correct else 0.0
    macro_file_recall_correct = sum(all_file_rec_correct) / len(all_file_rec_correct) if all_file_rec_correct else 0.0
    macro_file_f1_correct = sum(all_file_f1_correct) / len(all_file_f1_correct) if all_file_f1_correct else 0.0
    ...
    print(f"[File Level within Correct Directories] Macro Precision: {macro_file_precision_correct:.3f}")
    print(f"[File Level within Correct Directories] Macro Recall: {macro_file_recall_correct:.3f}")
    print(f"[File Level within Correct Directories] Macro F1: {macro_file_f1_correct:.3f}")
    ...
    all_results = {
        ...
        'file_level_within_correct_directories': {
            'macro_precision': macro_file_precision_correct, 
            'macro_recall': macro_file_recall_correct, 
            'macro_f1': macro_file_f1_correct, 
            'per_proposal': file_results_within_correct_dirs
        }
    }
    ...
    proposals_with_file_links_correct = sum(1 for res in file_results_within_correct_dirs.values() if res['precision'] > 0.0)
    ...
    md_lines.append(f"## File Level within Correct Directories Macro Metrics\n")
    md_lines.append(f"- **Number of Processed Proposals**: {len(llm_outputs)}")
    md_lines.append(f"- **Number of Proposals with at least one correct file (precision > 0)**: {proposals_with_file_links_correct}")
    md_lines.append(f"- **Macro Precision**: {macro_file_precision_correct:.3f}")
    md_lines.append(f"- **Macro Recall**: {macro_file_recall_correct:.3f}")
    md_lines.append(f"- **Macro F1**: {macro_file_f1_correct:.3f}\n")
    ...
    for proposal_id in file_results:
        ...
        # File-level within correct directories for this proposal
        file_res_correct = file_results_within_correct_dirs[proposal_id]
        md_lines.append(f"#### File Level within Correct Directories Results\n")
        file_found_correct = file_res_correct['tp']
        file_total_correct = file_res_correct['tp'] + file_res_correct['fn']
        md_lines.append(f"| **Precision** | **Recall** | **F1-Score** | **Found/Total** |")
        md_lines.append(f"|-----------|--------|----------|-------------|")
        md_lines.append(f"| {file_res_correct['precision']*100:.1f}% | {file_res_correct['recall']*100:.1f}% | {file_res_correct['f1']*100:.1f}% | {file_found_correct}/{file_total_correct} |")
        md_lines.append(f"\n##### Correct Directories Used for Evaluation\n")
        correct_dirs = file_res_correct['correct_directories']
        if correct_dirs:
            md_lines.append(f"**Correct Directories ({len(correct_dirs)}):**")
            for d in sorted(correct_dirs):
                md_lines.append(f"- `{d}`")
        else:
            md_lines.append("**No correct directories found** - This proposal had no correctly predicted directories, so file-level evaluation within correct directories is not applicable.")
        md_lines.append("")
