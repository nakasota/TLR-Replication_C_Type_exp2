#!/usr/bin/env python3
"""
Method v8: Function Level Localization Evaluation

This script evaluates the function level localization results.
It includes evaluation results from all previous stages:
1. Directory and file localization
2. File-level link decision (NEW in v8)
3. Function level localization

The evaluation provides comprehensive metrics across all processing stages.
"""
...
def main():
    ...
    # --- NEW: Append file-level link decision and directory/file localization results ---
    # Get selected file-level link decision output from function-level llm_outputs.json
    selected_file_level_link_decision = None
    try:
        with open(function_level_llm_output_path, 'r') as f:
            func_data = json.load(f)
        selected_file_level_link_decision = func_data.get("selected_file_level_link_decision_output", None)
        if selected_file_level_link_decision:
            print(f"Found selected file-level link decision output: {selected_file_level_link_decision}")
    except Exception as e:
        print(f"Warning: Could not read selected file-level link decision output: {e}")
    
    # Load file-level link decision results
    file_link_eval = {}
    if selected_file_level_link_decision:
        file_link_output_dir = Path(__file__).parent.parent / 'file_level_link_decision' / 'output' / selected_file_level_link_decision
        file_link_eval_path = file_link_output_dir / 'file_level_evaluation.json'
        if file_link_eval_path.exists():
            try:
                with open(file_link_eval_path, 'r') as f:
                    eval_data = json.load(f)
                file_link_eval = eval_data
                print(f"Loaded file-level link decision results from {file_link_eval_path}")
            except Exception as e:
                print(f"Warning: Could not load file-level link decision results: {e}")
        else:
            print(f"Warning: File-level link decision evaluation results not found at {file_link_eval_path}")
            print("Please run method_v8/file_level_link_decision/evaluate.py first to generate these results.")
    
    # Load directory and file-level results from directory_and_file_localization
    # We need to find which directory_and_file_localization output was used by file_level_link_decision
    dir_eval = {}
    file_eval = {}
    if selected_file_level_link_decision:
        # Try to find the latest directory_and_file_localization results
        dir_file_output_root = Path(__file__).parent.parent / 'directory_and_file_localization' / 'output'
        if dir_file_output_root.exists():
            latest_dir_file_output = None
            for dir_path in sorted(dir_file_output_root.iterdir(), reverse=True):
                if dir_path.is_dir():
                    latest_dir_file_output = dir_path
                    break
            
            if latest_dir_file_output:
                dir_file_eval_path = latest_dir_file_output / 'evaluation_results.json'
                if dir_file_eval_path.exists():
                    try:
                        with open(dir_file_eval_path, 'r') as f:
                            eval_data = json.load(f)
                        dir_eval = eval_data.get('directory_level', {})
                        file_eval = eval_data.get('file_level', {})
                        print(f"Loaded directory and file level results from {dir_file_eval_path}")
                    except Exception as e:
                        print(f"Warning: Could not load directory/file level results: {e}")
                else:
                    print(f"Warning: Directory/File level evaluation results not found at {dir_file_eval_path}")
            else:
                print("Warning: No directory_and_file_localization output found")
        else:
            print("Warning: directory_and_file_localization output directory not found")
            print("Please run method_v8/directory_and_file_localization/evaluate.py first to generate these results.")
    ...
    # File-level link decision macro metrics (NEW)
    md_lines.append(f"## File-Level Link Decision Macro Metrics\n")
    md_lines.append(f"- **Number of Processed Proposals**: {file_link_eval.get('proposals_with_data', function_eval['num_processed_proposals'])}")
    if file_link_eval:
        proposals_with_links = file_link_eval.get('proposals_with_data', 0)
        md_lines.append(f"- **Number of Proposals with at least one correct link (precision > 0)**: {proposals_with_links}")
    md_lines.append(f"- **Macro Precision**: {file_link_eval.get('average_precision', 0):.3f}")
    md_lines.append(f"- **Macro Recall**: {file_link_eval.get('average_recall', 0):.3f}")
    md_lines.append(f"- **Macro F1**: {file_link_eval.get('average_f1', 0):.3f}\n")
