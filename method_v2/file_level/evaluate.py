...
def main():
    ...
    for proposal_id, pred_dirs in dir_llm_outputs.items():
        gt_data = gt.get(proposal_id, {})
        gt_dirs = gt_data.get('dirs', set()) if isinstance(gt_data, dict) else set()
    ...
    all_prec_f_correct, all_rec_f_correct, all_f1_f_correct = [], [], []
    file_results_within_correct_dirs = {}
    
    for proposal_id, pred_files in file_llm_outputs.items():
        gt_data = gt.get(proposal_id, {})
        gt_files = gt_data.get('files', set()) if isinstance(gt_data, dict) else set()
        ...
        # File-level evaluation within correct directories
        pred_dirs = dir_llm_outputs.get(proposal_id, set())
        gt_dirs = gt_data.get('dirs', set()) if isinstance(gt_data, dict) else set()
        precision_correct, recall_correct, f1_correct, tp_correct, fp_correct, fn_correct = compute_file_metrics_within_correct_directories(
            pred_files, gt_files, pred_dirs, gt_dirs
        )
        file_results_within_correct_dirs[proposal_id] = {
            'precision': precision_correct,
        ...
        all_prec_f_correct.append(precision_correct)
        all_rec_f_correct.append(recall_correct)
        all_f1_f_correct.append(f1_correct)
    ...
    # Macro metrics for file-level within correct directories
    macro_precision_f_correct = sum(all_prec_f_correct) / len(all_prec_f_correct) if all_prec_f_correct else 0.0
    macro_recall_f_correct = sum(all_rec_f_correct) / len(all_rec_f_correct) if all_rec_f_correct else 0.0
    macro_f1_f_correct = sum(all_f1_f_correct) / len(all_f1_f_correct) if all_f1_f_correct else 0.0
    ...
    print(f"[File Level within Correct Directories] Macro Precision: {macro_precision_f_correct:.3f}")
    print(f"[File Level within Correct Directories] Macro Recall: {macro_recall_f_correct:.3f}")
    print(f"[File Level within Correct Directories] Macro F1: {macro_f1_f_correct:.3f}")
    ...
    with open(output_dir / 'evaluation_results.json', 'w') as f:
        json.dump({
            ...
            'file_level_within_correct_directories': {'macro_precision': macro_precision_f_correct, 'macro_recall': macro_recall_f_correct, 'macro_f1': macro_f1_f_correct, 'per_proposal': file_results_within_correct_dirs}
        }, f, indent=2)
    ...
    proposals_with_links_f_correct = sum(1 for res in file_results_within_correct_dirs.values() if res['precision'] > 0.0)
    md_lines = []
    ...
    md_lines.append(f"- **Number of Processed Proposals**: {len(file_llm_outputs)}")
    md_lines.append(f"- **Number of Proposals with at least one correct link (precision > 0)**: {proposals_with_links_f_correct}")
    md_lines.append(f"- **Macro Precision**: {macro_precision_f_correct:.3f}")
    md_lines.append(f"- **Macro Recall**: {macro_recall_f_correct:.3f}")
    md_lines.append(f"- **Macro F1**: {macro_f1_f_correct:.3f}\n")
    ...
    for pid, res in file_results.items():
        ...
        for d in sorted(pred_files):
            ...
            md_lines.append(f"- {mark} `{d}`")
        md_lines.append("")
    for pid, res in file_results_within_correct_dirs.items():
        md_lines.append(f"\n### 📊 **Proposal #{pid} (File Level within Correct Directories)**\n")
        found = res['tp']
        total = res['tp'] + res['fn']
        md_lines.append(f"| **Precision** | **Recall** | **F1-Score** | **Found/Total** |")
        md_lines.append(f"|-----------|--------|----------|-------------|")
        ...
        correct_dirs = res['correct_directories']
        if correct_dirs:
    ...
    summary = {
        ...
        "file_level_within_correct_directories": {
            "macro_precision": macro_precision_f_correct,
            "macro_recall": macro_recall_f_correct,
            "macro_f1": macro_f1_f_correct,
            "num_processed_proposals": len(file_llm_outputs)
        }
    }
